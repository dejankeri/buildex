...
| **Store** | A client of the plugin marketplaces your agent already has. Three ship with the app, so a fresh repo has a full shelf on first run, and you can add your own. Installing is your agent's own CLI doing it (`claude plugin install … --scope user`) — BuildEx unpacks nothing, and no plugin file or skill scaffold lands in your repo, so there is nothing of ours in there to overwrite. What installing does write is the gate and the agent's context file, which is the part a marketplace can't carry. Seven business apps are curated and say so on the shelf. |
