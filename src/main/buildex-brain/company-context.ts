...
function renderApps(apps: InstalledAppSummary[]): string[] {
  ...
  const lines = [
    ...
    'well — read the skill before improvising. Skills live in `.buildex/skills/`',
 ...
 */
export function renderCompanyContext(scan: BrainScan, apps: InstalledAppSummary[] = []): string {
  const lines: string[] = [
    '# Company context',
    '',
...
export function syncCompanyContext(
  ...
  const nextContext = renderCompanyContext(scan, apps)
