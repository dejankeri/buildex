...
export function configureDevUserDataPath(isDev: boolean): void {
  ...
  // Why: without a dev-only path, pnpm dev overwrites the packaged app's runtime pointer under userData and breaks the buildex CLI.
  app.setPath('userData', join(app.getPath('appData'), 'buildex-dev'))
}
