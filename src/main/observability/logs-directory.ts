...
// macOS convention is `~/Library/Application Support/BuildEx/logs/`; Windows and
...
function getUserDataDir(): string {
  try {
    ...
    const home = homedir()
    if (platform() === 'darwin') {
      return join(home, 'Library', 'Application Support', 'BuildEx')
    }
    if (platform() === 'win32') {
      return join(process.env.APPDATA ?? home, 'BuildEx')
    }
    return join(home, '.config', 'BuildEx')
  }
}
