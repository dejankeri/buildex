...
const ORCA_PRODUCT_URL = 'https://github.com/dejankeri/buildex'
...
if [[ "\${1:-}" == "pr" && "\${2:-}" == "create" ]]; then
  footer="\${ORCA_GH_PR_FOOTER:-Made with [BuildEx](https://github.com/dejankeri/buildex) 🏗}"
...
if [[ "\${1:-}" == "issue" && "\${2:-}" == "create" ]]; then
  footer="\${ORCA_GH_ISSUE_FOOTER:-Made with [BuildEx](https://github.com/dejankeri/buildex) 🏗}"
  if has_passthrough_create_args "$@"; then
    PATH="$real_path" exec "$real_gh" "$@"
  fi
  if ! has_noninteractive_create_args "$@"; then
...
if ($isPrCreate) {
  ...
  if ($createdUrl) {
    ...
    $footer = if ($env:ORCA_GH_PR_FOOTER) { $env:ORCA_GH_PR_FOOTER } else { 'Made with [BuildEx](https://github.com/dejankeri/buildex) 🏗' }
...
if ($isIssueCreate) {
  ...
  if ($createdUrl) {
    ...
    $footer = if ($env:ORCA_GH_ISSUE_FOOTER) { $env:ORCA_GH_ISSUE_FOOTER } else { 'Made with [BuildEx](https://github.com/dejankeri/buildex) 🏗' }
    if ($null -ne $body -and $body -notmatch [Regex]::Escape($footer)) {
      $tmpFile = [System.IO.Path]::GetTempFileName()
      try {
        $trimmed = $body.TrimEnd("${POWERSHELL_TICK}r", "${POWERSHELL_TICK}n")
