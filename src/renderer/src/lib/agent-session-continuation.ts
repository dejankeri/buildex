...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `BuildEx pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior BuildEx session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest BuildEx status hints:', ...statusHints] : []),
    '',
