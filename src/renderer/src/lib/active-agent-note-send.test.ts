...
describe('active agent note send', () => {
  ...
  it('uses selected-target failure wording for explicit note targets', () => {
    ...
    expect(
      ...
      'The notes may already be pasted in the selected terminal, but BuildEx could not submit them.'
    )
    expect(activeAgentNotesSendFailureMessage('partial-submit-failed')).toBe(
      'The notes may already be pasted in the active terminal, but BuildEx could not submit them.'
    )
  })
