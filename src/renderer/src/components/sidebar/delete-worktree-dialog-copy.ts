  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from BuildEx. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from BuildEx. Git worktrees will also be removed from git and disk; folder workspaces will only remove the BuildEx workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from BuildEx. The project folder on disk will not be deleted.'
