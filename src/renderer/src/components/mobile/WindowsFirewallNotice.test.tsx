...
describe('WindowsFirewallNotice', () => {
  ...
  it('reports an overriding Block rule and stays actionable if repair cannot clear it', async () => {
    ...
    expect(await screen.findByText(/Windows may be blocking BuildEx Mobile/i)).toBeInTheDocument()
    ...
    await waitFor(() => expect(getWindowsFirewallStatus).toHaveBeenCalledTimes(2))
    expect(screen.getByText(/Windows may be blocking BuildEx Mobile/i)).toBeInTheDocument()
  })
  ...
  it('ignores a stale inspection that resolves after a newer result', async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByText(/Windows may be blocking BuildEx Mobile/i)).not.toBeInTheDocument()
    )
