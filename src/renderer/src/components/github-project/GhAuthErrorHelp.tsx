...
export function buildRemediation(
  ...
  if (!diag.ghAvailable) {
    return {
      ...
      detail:
        'BuildEx uses `gh` to talk to GitHub Projects. Install it from cli.github.com, then sign in.',
      commands: [
        {
          label: translate(
            'auto.components.github.project.GhAuthErrorHelp.9c2da6353b',
  ...
  if (active?.envToken) {
    ...
    return {
      ...
      detail: IS_WINDOWS
        ? `Find where \`${varName}\` is set (System or User environment variables, or your PowerShell profile), remove it, then restart BuildEx so the new environment is picked up.${fallback}`
        : `Find where \`${varName}\` is exported (commonly \`~/.zshrc\`, \`~/.zshenv\`, \`~/.bashrc\`, \`~/.profile\`, or your shell's secrets manager), remove it, then restart BuildEx so the new environment is picked up.${fallback}`,
  ...
  if (diag.envTokenInProcess && (!active || diag.missingScopes.length > 0)) {
    ...
    return {
      summary: `BuildEx inherited \`${varName}\` from your shell, and \`gh\` is using that token. \`gh auth refresh\` doesn't apply to env-supplied tokens.`,
