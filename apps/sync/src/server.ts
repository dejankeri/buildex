...
/**
 * Store construction seam, overridable so a test can hold a direct reference to the real
 * `ControlPlaneStore` instance `createServices` builds internally and assert on it after
 * `close()` - e.g. that a post-close operation throws. Defaults to the real constructor, so
 * `createServices(config)` is unchanged for every existing caller.
 */
...
export async function createServices(
  ...
  // The store opens a real SQLite handle in its constructor, before this function has anything
  ...
  // The store must close on shutdown - a leaked SQLite handle is exactly what blocked cleanup
  // before ee770eb. Idempotent: shutdown can race a signal with an error path, so a second call
