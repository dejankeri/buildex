...
/** The real Supabase (GoTrue) auth client: PKCE OAuth over `/auth/v1/authorize` and
 *  `/auth/v1/token?grant_type=pkce`. Google is the only provider; there is no anonymous path (see
 *  wiring.ts - a workspace is signed in or fully local, nothing in between).
 *  Verified against the live project on 2026-07-25: `/auth/v1/authorize?provider=google` 302s to
 *  accounts.google.com with the loopback redirect allow-listed. */
