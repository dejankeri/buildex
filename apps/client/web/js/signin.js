...
// Reached from the left-rail "Back up & sync" pill (projects.js), the title-bar sync dot (boot.js)
// and the pending tray's save card (pending.js), all shown only while the workspace has no connected
// account. Google is the only way in: the setup-code flow it used to hand off to was removed - a
// workspace is signed in or fully local, and an operator should never be asked for a company URL or
 ...
 * explaining the account needs attention shows inline and the modal stays up. Any other error shows
 * inline the same way, for the operator to retry.
 * @returns {void}
 */
function startSignIn() {
  ...
  const draw = () => {
    ...
    // unreachable server; the 4xx/error case is read from `res.error` in the branch below.
    const signIn = async (body, btn, busyLabel) => {
      ...
      if (res && res.state === "connected") {
        ...
        if (res && res.state === "needs-help") {
          ...
          // that raw internal string. Staying local is a real end state, so say that, not "try again".
          error = "Backing up isn't available in this build - your work stays on this machine.";
        } else {
